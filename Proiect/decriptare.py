...
def inv_sub_bytes(s):
    for i in range(4):
        for j in range(4):
            ...
            s[i][j]=inv_sbox[row][column]


def inv_shift_rows(s):
    #prima linie ramane neschimbata, urmatoarele se deplaseaza la dreapta cu n pozitii, unde n e numarul randului
    s[1] = s[1][-1:] + s[1][:-1]
    s[2] = s[2][-2:] + s[2][:-2]
    s[3] = s[3][-3:] + s[3][:-3]
...
#0e=xTimes(xTimes(xTimes(a)))^xTimes(xTimes(a))^xTimes(a)
#0b=xTimes(xTimes(xTimes(b)))^xTimes(b)^b
#0d=xTimes(xTimes(xTimes(c)))^xTimes(xTimes(c))^c
#09=xTimes(xTimes(xTimes(d)))^d
def inv_mix_a_column(s):
    ...
    s[0]=xTimes(xTimes(xTimes(a)))^xTimes(xTimes(a))^xTimes(a) ^ xTimes(xTimes(xTimes(b)))^xTimes(b)^b ^ xTimes(xTimes(xTimes(c)))^xTimes(xTimes(c))^c ^ xTimes(xTimes(xTimes(d)))^d
    s[1]=xTimes(xTimes(xTimes(a)))^a ^ xTimes(xTimes(xTimes(b)))^xTimes(xTimes(b))^xTimes(b) ^ xTimes(xTimes(xTimes(c)))^xTimes(c)^c ^ xTimes(xTimes(xTimes(d)))^xTimes(xTimes(d))^d
    s[2]=xTimes(xTimes(xTimes(a)))^xTimes(xTimes(a))^a ^ xTimes(xTimes(xTimes(b)))^b ^ xTimes(xTimes(xTimes(c)))^xTimes(xTimes(c))^xTimes(c) ^ xTimes(xTimes(xTimes(d)))^xTimes(d)^d
    s[3]=xTimes(xTimes(xTimes(a)))^xTimes(a)^a ^ xTimes(xTimes(xTimes(b)))^xTimes(xTimes(b))^b ^ xTimes(xTimes(xTimes(c)))^c ^ xTimes(xTimes(xTimes(d)))^xTimes(xTimes(d))^xTimes(d)


def inv_mix_columns(s):
    for i in range(4):  
        column = [s[j][i] for j in range(4)]  # extrag coloana
        inv_mix_a_column(column)
...
def key_expansiond(key):
    ...
    for i in range(1,Nr+1):
        ...
        print('w3 dupa rotate')
        for element in w3_new:
            print(hex(element), end=' ')
        print()
        #iau din sbox
    ...
    print("key_expanded:")
    #print(key_expanded)
    for row in key_expanded:
        for element in row:
           print(['0x{:02x}'.format(byte) for byte in element])
        print()
    return key_expanded


      
def invcipher(input_block, Nr, w):
    state = input_block
    for row in state:
            line = ' '.join(['0x{:02x}'.format(element) for element in row])
            print(line)
